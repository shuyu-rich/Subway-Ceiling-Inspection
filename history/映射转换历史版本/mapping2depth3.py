"""
L相机：lv_mapping2deep(铝方通)、gd_mapping2deep(勾搭铝板)、map_mapping2deep(铝板及其他)
S相机：S_mappiing2deep
非极大值抑制：nms(联合模型检测后去重)
"""
...
def gd_mapping2depth(x1, y1, x2, y2):
    ...
    if is_horizontal:
        ...
        if ratio >= 0.6:  # 如果框太方了就得多处理一下
            c_w = b_w - (b_w * (1 - ratio))
            c_h = b_h - (b_h * ratio)
            depth_x1, depth_y1 = b_depth_x1 + int(c_w / 1.5), b_depth_y1 + int(c_h / 1.2)
            depth_x2, depth_y2 = b_depth_x2 - int(c_w / 1.5), b_depth_y2 - int(c_h / 1.2)
        else:
    ...
    # 竖的比横的数值大，is_horizontal为False，竖向标签框
        ...
        if ratio >= 0.6:  # 如果框太方了就得多处理一下
            ...
            print("c_w", c_w)
            c_h = b_h - (b_h * (1 - ratio))
            depth_x1, depth_y1 = b_depth_x1 - int(c_w / 0.5), b_depth_y1 + int(c_h / 1.2)
            depth_x2, depth_y2 = b_depth_x2 + int(c_w / 0.5), b_depth_y2 - int(c_h / 1.2)
        else:
...
def mapping2depth(x1, y1, x2, y2):
	...
	只用于计算用的位置坐标，不能用来画框，请与用来画框的位置坐标做区分
	'''
    global depth_y1, depth_y2, depth_x2, depth_x1
    c_box = []
    d_height, d_width = 1405 - 50, 1910 - 80
    weight, height = 2000, 1500
    d_x1 = abs(x1 + (x1 * (0.02 * (x1 / weight))))
    ...
    if is_horizontal:
        ...
        if b_w > 100 or b_h > 100:  # 判断框大小，如果框太小了，就不再截了
            if ratio >= 0.6:  # 如果框太方了就得多处理一下
                ...
                c_w = b_w - (b_w * ratio)  # 横的长，多截一点，少留一点
                c_h = b_h * 1.8  # 竖的太短，直接不截掉，全部保留竖着的像素
                depth_x1, depth_y1 = b_depth_x1 + int(c_w / 2), b_depth_y1 - int(c_h / 2)
                depth_x2, depth_y2 = b_depth_x2 - int(c_w / 2), b_depth_y2 + int(c_h / 2)
            elif ratio <= 0.1:
                c_w = b_w - (b_w * ratio)  # 横的长，多截一点，少留一点
    ...
    # 竖的比横的数值大，is_horizontal为False，竖向标签框
        ...
        if b_w > 100 or b_h > 100:  # 判断框大小，如果框太小了，就不再截了
            if ratio >= 0.6:  # 如果框太方了就得多处理一下
                ...
                c_w = b_w * 1.8  # 横的太短，直接不截掉，全部保留竖着的像素
                c_h = b_h - (b_h * ratio)  # 竖的长，多截一点，少留一点
                depth_x1, depth_y1 = b_depth_x1 - int(c_w / 2), b_depth_y1 + int(c_h / 2)
                depth_x2, depth_y2 = b_depth_x2 + int(c_w / 2), b_depth_y2 - int(c_h / 2)
            elif ratio < 0.1:
    ...
    for cut in counts:
        c_x1, c_y1, c_x2, c_y2, c_mean, c_shape = cut
        if 80 < c_y1 and c_y2 < 1230 and c_x1 > 120:
            # if c_y2 <= 857 :
...
def S_mappiing2depth(x1, y1, x2, y2):
    """
    	将RGB图的坐标映射到深度图的坐标，需要先进行检测，得到检测框的位置坐标才能进行映射，在遍历获取xyxy坐标循环内调用
	接深度图的z轴计算、裂缝宽度X/Y轴计算
	:param x1: RGB图的左上x坐标
    ...
    """
    # 获取图像的高度和宽度
    global depth_x2, depth_y2, depth_x1, depth_y1
    c_box = []

    d_height, d_width = 1536, 2048
    weight, height = 2008, 1518
    if x1 < 1300 and y1 < 600:
        print("left",f"({x1} + ({x1} * (0.00001 * ({x1} / {weight}))))")
        d_x1 = abs(x1 + (x1 * (0.00001 * (x1 / weight))))
        # d_x1 = x1
        d_y1 = abs(y1 + (y1 * (0.035 * (y1 / height))))
        d_x2 = abs(x2 + (x2 * (0.000015 * (x2 / weight))))
        # d_x2 = x2
    ...
    if is_horizontal:
        ...
        for i in range(1, count):
            ...
            dw_depth_x2, dw_depth_y2 = depth_x2 + (mean * i), depth_y2
            if up_depth_x1 > b_depth_x1 + 10:
                # print(up_depth_x1,"----------------------", b_depth_x1+10)
                counts.append([up_depth_x1, up_depth_y1, up_depth_x2, up_depth_y2, mean, shape])
            if dw_depth_x2 < b_depth_x2 - 10:
                # print(dw_depth_x2,"----------------------",b_depth_x2-10)
                counts.append([dw_depth_x1, dw_depth_y1, dw_depth_x2, dw_depth_y2, mean, shape])
    # 竖的比横的数值大，is_horizontal为False，竖向标签框
    else:
        # 纵向标签框
        ...
        for i in range(1, count):
            ...
            dw_depth_x2, dw_depth_y2 = depth_x2, depth_y2 + (mean * i)
            if up_depth_y1 > b_depth_y1 + 10:
                # print(up_depth_y1,"----------------------", b_depth_y1+10)
                counts.append([up_depth_x1, up_depth_y1, up_depth_x2, up_depth_y2, mean, shape])
            if dw_depth_y2 < b_depth_y2 - 10:
                # print(dw_depth_y2,"----------------------",b_depth_y2-10)
                counts.append([dw_depth_x1, dw_depth_y1, dw_depth_x2, dw_depth_y2, mean, shape])
    for cut in counts:
        c_x1, c_y1, c_x2, c_y2, c_mean, c_shape = cut
        if c_y2 < 1400 and c_y1 > 150 and c_x2 < 1950:
            c_box.append([c_x1, c_y1, c_x2, c_y2, c_mean, c_shape])
    return c_box
