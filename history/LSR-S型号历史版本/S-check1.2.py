...
class ImageCaptureThread(QThread):
    ...
    def load_model(self, model_name):
        ...
        weight_paths = {
            '铝板': 'D:\\PythonCode\\yolov5\\runs\\train\\all\\716fanglvban.pt',
...
class ZoomWindow(QDialog):
    ...
    def save_image(self):
        ...
        ceiling_type_number = ceiling_type_mapping.get(self.model_name, 0)  # 默认值为 0
        ...
        # 将 self.label_stats 中的标签转换为对应的数字
        damage_type_data = [damage_type_mapping.get(label, 0) for label in self.label_stats]
        ...
        new_data_entry = {
            ...
            "damageDesc": "墙面有裂缝",
            "damageType":  damage_type_data if len(damage_type_data) > 1 else damage_type_data[0],  # 根据长度确定是整数还是整数列表
        ...
        # 插入日志记录到 file_load_log 表
        self.parent_window.db_cursor.execute('''
           INSERT INTO file_load_log (file_name, file_path, load_start_time, load_end_time, duration_ms, status, loaded_by, file_size_bytes, records_loaded)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
           ''', (
            image_file_name,
            image_save_path,
            current_time,
            current_time,
            0,  # Duration, could be updated if needed
            'Success',
            'System',  # Replace with actual user/system
            os.path.getsize(image_save_path),
            1  # We are inserting one record
        ))
        log_id = self.parent_window.db_cursor.lastrowid
        self.parent_window.db_connection.commit()
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        # 向下拉框添加模型选项
        model_options = ["铝板", "栅格", "反光冲孔板", "冲孔板", "勾搭铝板", "拉网", "铝条板", "栅格加网", "铝方通",
                         "矿棉板", "垂片"]
