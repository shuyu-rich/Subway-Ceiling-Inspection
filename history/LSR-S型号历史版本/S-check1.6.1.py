...
class ImageCaptureThread(QThread):
    ...
    def load_model(self, model_name):
        ...
        weight_paths = {
            ...
            '勾搭铝板': 'D:/PythonCode/yolov5/runs/train/all/807gouda.pt',
            '垂片': 'D:/PythonCode/yolov5/runs/train/all/511lvfangtong.pt',
            '高铝板': 'D:/PythonCode/yolov5/runs/train/all/820gaolvban.pt'
        }
        ...
        for cuofengg_box in cuofengg_boxes:
            ...
            for i in range(len(inside_cuofeng_boxes) - 1):
                ...
                if vertical_list >= 10 or vertical_distance >= 3.0:
                    ...
                    global_lc_cf_conditions.append(self.detected_label)
                    global_lc_lf_conditions.sort(key=lambda x: x['column_index'])
                    if self.detected_labels:
                        max_lc = max(label['LC'] for label in self.detected_labels if 'LC' in label)
...
class ZoomWindow(QDialog):
    ...
    def save_image(self):
        ...
        location_text = self.location.text()
        ...
        for condition in global_lc_lf_conditions:
            if condition['LC'] > 3 or condition['LF'] > 2:
                if "LC" not in damage_type:
                    damage_type.append("LC")
                if "LF" not in damage_type:
                    damage_type.append("LF")
                lc_value = float(condition['LC']) if isinstance(condition['LC'], np.float32) else condition['LC']
                lf_value = float(condition['LF']) if isinstance(condition['LF'], np.float32) else condition['LF']
                damage_data.append(f"LC:{lc_value:.2f}, LF:{lf_value:.2f}")
        ...
        for condition in global_lc_cf_conditions:
            ...
            if lc_value > 3 or cf_value > 2:
                if "LC" not in damage_type:
                    damage_type.append("LC")
                if "CF" not in damage_type:
                    damage_type.append("CF")
                damage_data.append(f"LC:{lc_value:.2f}, CF:{cf_value:.2f}")
        ...
        if damage_type and damage_data:
            ...
            new_data_entry = {
                ...
                "damageDesc": "墙面有裂缝",
                "damageType": damage_type_str,  # 保存类别LC, LF, CF
            ...
            self.parent_window.db_cursor.execute('''
               ...
               depart_id, depart_name, inspection_time, damageDesc, damageType, damageDetail, damageLocation, picName, ceilingType, isDisposed, measures, damageData)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)
               ''', (
