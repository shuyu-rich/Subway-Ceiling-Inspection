...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.fixed_port_name = 'COM8'  # 请将此处替换为你的实际串口名称，如 'COM3' 或 '/dev/ttyUSB0'
        self.baudrate = 19200
    ...
    def update_measurement_data(self):
        try:
            if self.is_measurement_started and not self.is_measurement_paused:
                ...
                # 平滑测量数据
                smoothed_distance = self.moving_average(self.distance_meters)
                self.data_display.append(f"测量距离 (米): {smoothed_distance:.2f} 米")
