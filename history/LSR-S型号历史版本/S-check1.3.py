...
class YourClass(QObject):
    ...
    def __init__(self):
        ...
        self.ser = None
        self.connect_to_serial("COM4")  # 自动连接到指定的串口
...
class ImageCaptureThread(QThread):
    ...
    def load_model(self, model_name):
        ...
        weight_paths = {
            ...
            '铝方通': 'D:/PythonCode/yolov5/runs/train/all/717lvfangtong.pt',
            '勾搭铝板': 'D:/PythonCode/yolov5/runs/train/all/cuowei.pt',
            '垂片': 'D:/PythonCode/yolov5/runs/train/all/717lvfangtong.pt'
        }
    ...
    def process_detections(self, results, color_frame):
        ...
        for cuofengg_box in cuofengg_boxes:
            ...
            for i in range(len(inside_cuofeng_boxes) - 1):
                ...
                if vertical_list >= 10 or depth_difference >= 3.0:
                    self.alarmStatusChanged.emit(True)
                ...
                self.draw_boxes(color_frame, [inside_cuofeng_boxes[i], inside_cuofeng_boxes[i + 1]],
                                [vertical_list], [y_diff_mm], [depth_difference])
...
class MyWindow(QMainWindow):
    ...
    def get_line_details(self, line_name):
        ...
        self.db_cursor.execute("SELECT line_id FROM facilities WHERE line_name=?", (line_name,))
        result = self.db_cursor.fetchone()
        return result[0], line_name if result else (None, line_name)

    def get_station_details(self, station_name):
        # 根据 station_name 从数据库或其他来源获取 station_id 和 station_name
        self.db_cursor.execute("SELECT station_id FROM facilities WHERE station_name=?", (station_name,))
        result = self.db_cursor.fetchone()
        return result[0], station_name if result else (None, station_name)

    def get_area_details(self, area_name):
        # 根据 area_name 从数据库或其他来源获取 area_id 和 area_name
        self.db_cursor.execute("SELECT celling_id FROM facilities WHERE celling_name=?", (area_name,))
        result = self.db_cursor.fetchone()
    ...
    def update_measurement_data(self):
        try:
            if self.is_measurement_started and not self.is_measurement_paused:
                ...
                tolerance = 0.05
                if self.distance_meters > 1.26 + tolerance:
                ...
                elif self.distance_meters < 1.26 - tolerance:
