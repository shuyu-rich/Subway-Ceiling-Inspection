...
class ImageCaptureThread(QThread):
    ...
    def load_model(self, model_name):
        ...
        weight_paths = {
            ...
            '铝条板': 'D:/PythonCode/yolov5/runs/train/all/711ltb.pt',
            ...
            '高铝板': 'D:/PythonCode/yolov5/runs/train/all/gaolvban.pt'
        }
        ...
        for *xyxy, conf, cls in res:
            ...
            try:
                if self.selected_model == "勾搭铝板":
                    ...
                    counts = S_gd_mappiing2depth(x1, y1, x2, y2)
                else:
                    # counts = mapping2depth(x1, y1, x2, y2)
                    # 调用映射坐标方法
                    counts = S_mappiing2depth(x1, y1, x2, y2)  # S相机映射方法
            ...
            # print(f"{fov_height_pixels} * {midd} / {image_height} = {vertical_distance3}")
            detected_label = {
...
class ZoomWindow(QDialog):
    ...
    def init_ui(self):
        ...
        self.disposal_dis.addItems(["是", "否"])
        self.disposal_1 = QLineEdit("处理措施")
        line_edit_style = """
            QLineEdit {
    ...
    def save_image(self):
        ...
        alarmed_labels_set_lf = set()  # 使用集合去重
        alarmed_labels_set_cf = set()  # 使用集合去重

        # 处理global_lc_lf_conditions
        for label in global_lc_lf_conditions:
            if label['LC'] > 3:
                alarmed_labels_set_lf.add("1")
            if label['LF'] > 2:  # 假设 LF 的报警条件是 > 10
                alarmed_labels_set_lf.add("2")

        # 处理global_lc_cf_conditions
        for label in global_lc_cf_conditions:
            if label['LC'] > 3:
                alarmed_labels_set_cf.add("1")
            if label['CF'] > 3:  # 假设 CF 的报警条件是 > 10
                alarmed_labels_set_cf.add("3")

        # 根据具体情况生成damageType字符串
        if alarmed_labels_set_lf and not alarmed_labels_set_cf:
            damage_type_str = ",".join(sorted(alarmed_labels_set_lf))
        elif not alarmed_labels_set_lf and alarmed_labels_set_cf:
            damage_type_str = ",".join(sorted(alarmed_labels_set_cf))
        else:
            damage_type_str = ",".join(sorted(alarmed_labels_set_lf.union(alarmed_labels_set_cf)))
        ...
        self.parent_window.db_cursor.execute('''
           ...
           depart_id, depart_name, inspection_time, damageDesc, damageType, damageDetail, damageLocation, picName, ceilingType, isDisposed, measures)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
           ''', (
            ...
            new_data_entry["damageDesc"],
            new_data_entry["damageType"],
            new_data_entry["damageDetail"],
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        # 向下拉框添加模型选项
        model_options = ["补丁铝板", "铝板", "栅格", "反光冲孔板", "冲孔板", "勾搭铝板", "拉网", "铝条板", "栅格加网", "铝方通", "矿棉板", "垂片"]
    ...
    def update_measurement_data(self):
        try:
            if self.is_measurement_started and not self.is_measurement_paused:
                ...
                tolerance = 0.05
                if self.distance_meters > 1.06 + tolerance:
                    ...
                    time.sleep(0.1)
                    self.your_class_instance.relay4_state_changed(False)
                elif self.distance_meters < 1.06 - tolerance:
                    self.your_class_instance.relay1_state_changed(False)
                    time.sleep(0.1)
                    self.your_class_instance.relay2_state_changed(False)
                    time.sleep(0.1)
                    ...
                    QMessageBox.information(self, "测量停止", "测距仪已测得1.2米，测量已停止。")
    ...
    def update_detection_result(self, detection_img):
        # 检查 detection_img 是否为 QImage 或可以从元组中提取 QImage
        if isinstance(detection_img, QImage):
            # 如果 detection_img 是 QImage，则直接使用
            pixmap = QPixmap.fromImage(detection_img)
            self.image_label2.setPixmap(pixmap)
            self.image_label2.setAlignment(Qt.AlignCenter)
        elif isinstance(detection_img, tuple) and len(detection_img) > 0 and isinstance(detection_img[0], QImage):
            # 如果 detection_img 是元组且第一个元素是 QImage，则从元组中提取 QImage
            image_to_display = detection_img[0]
            pixmap = QPixmap.fromImage(image_to_display)
            self.image_label2.setPixmap(pixmap)
            self.image_label2.setAlignment(Qt.AlignCenter)
        else:
            # 如果 detection_img 不是 QImage 也不是包含 QImage 的元组，则打印错误
            print(f"Invalid type for detection_img: {type(detection_img)}")
    ...
    def export_data(self):
        ...
        for row in latest_data:
            ...
            data = {
                "id": row[0],
                "inspector": row[1],
                "line_id": str(row[2]),  # 转换为字符串
                "line_name": row[3],
                "station_id": str(row[4]),  # 转换为字符串
                "station_name": row[5],
                "ceiling_id": str(row[6]),  # 转换为字符串
                "ceiling_name": row[7],
                "depart_id": str(row[8]),
                "depart_name": row[9],
