...
class ImageCaptureThread(QThread):
    ...
    def capture_images(self):
        ...
        detection_results, tankuang = self.detect_objects(color_image)
    ...
    def detect_objects(self, color_frame):
        ...
        if self.selected_model == "铝板":
            ...
            model_path = './yolov5'
            weight_path = './yolov5/runs/train/all/kuangmian.pt'
            ...
            # weight_path = 'C:/Users/20115/Desktop/yolov5/runs/train/exp4/weights/best.pt'
            weight_path = './models/heibai_ban3.pt'
            # 读取模型
            model = torch.hub.load(model_path, 'custom', weight_path, source='local')
            # 色彩图传入模型进行目标检测
            results = model(color_frame)
            ...
            # weight_path = 'C:/Users/20115/Desktop/yolov5/runs/train/拉网/best(1).pt'
            # weight_path = 'C:/Users/20115/Desktop/yolov5/runs/train/exp4/weights/best.pt'
            weight_path = './models/chang_lvban.pt'
            # 读取模型
            model = torch.hub.load(model_path, 'custom', weight_path, source='local')
            # 色彩图传入模型进行目标检测
            results = model(color_frame)
    ...
    def process_detections(self, results,color_frame):
        ...
        for cuofengg_box in cuofengg_boxes:
            ...
            for i in range(len(inside_cuofeng_boxes) - 1):
                ...
                # 映射到深度图
                depth_x1 = int(x1 * depth_width / image_width)
                depth_y1 = int(y1 * depth_height / image_height)
                depth_x2 = int(x2 * depth_width / image_width)
                depth_y2 = int(y2 * depth_height / image_height)
                ...
                # 矩阵之和，白一些的像素有多少
                summ = np.sum(thresh)
                # is_horizontal = abs(depth_x2 - depth_x1) > abs(depth_y2 - depth_y1)
                if is_horizontal:
                    # 横向排列
                    ...
                    if summ:
                        midd = ((w * h) - summ) // h
                        # 计算每排裂缝像素
                        # vertical_list = [depth_y2 - depth_y1 - i for i in row_sums]
                    ...
                    # 获取深度值并计算差值
                    try:
                        depth_y = (depth_y2+depth_y1) //2
                        depth_value_right_top = depth_frame1[int(depth_x1+80), int(depth_y+50)]
                        depth_value_left_top = depth_frame1[int(depth_x2+80), int(depth_y+50)]
                        x, y, depth_value_right_top = point_xyz_data[int(depth_y+50)][int(depth_x1+80)]
                        x, y, depth_value_left_top = point_xyz_data[int(depth_y + 50)][int(depth_x2+80)]
                        print("x加80，y+50, 去软件里看",depth_x1+80, depth_y+50, depth_x2+80, depth_y+50)
                        vertical_distance = self.calculate_vertical_distance(depth_value_right_top,
                                                                             depth_value_left_top)
                        cv2.rectangle(depth_frame, (int(depth_x1 - 3), int(depth_y - 3)), (int(depth_x1 + 3), int(depth_y + 3)),
                                      (0, 0, 255), 2)
                        cv2.rectangle(depth_frame, (int(depth_x2 - 3), int(depth_y - 3)), (int(depth_x2 + 3), int(depth_y + 3)),
                                      (0, 0, 255), 2)
                    except Exception as e:
                        print(f"超出边界，错误信息{e}")
                    ...
                    # 获取深度值并计算差值
                    try:
                        depth_x = (depth_x2 + depth_x1) // 2
                        depth_value_right_top = depth_frame1[int(depth_x + 80), int(depth_y1 + 50)]
                        depth_value_left_top = depth_frame1[int(depth_x + 80), int(depth_y2 + 50)]
                        x, y, depth_value_right_top = point_xyz_data[int(depth_y1 + 50)][int(depth_x + 80)]
                        x, y, depth_value_left_top = point_xyz_data[int(depth_y2 + 50)][int(depth_x + 80)]
                        print("x加80，y+50, 去软件里看", depth_x + 80, depth_y1 + 50, depth_x + 80, depth_y2 + 50)
                        ...
                        depth_difference = self.calculate_vertical_distance(depth_value_right_top,
                                                                             depth_value_left_top)
                        # cv2.rectangle(depth_frame, (int(depth_x1 - 3), int(depth_y - 3)),
                        #               (int(depth_x1 + 3), int(depth_y + 3)),
                        #               (0, 0, 255), 2)
                    ...
                    if vertical_list2 > 100:
                        pygame.mixer.music.load("D:/PycharmProjects/pythonProject/mp3/sp.mp3")  # 替换为你的音频文件路径
                        pygame.mixer.music.play()

                    if depth_difference > 3:
                        pygame.mixer.music.load("D:/PycharmProjects/pythonProject/mp3/sp.mp3")  # 替换为你的音频文件路径
                        pygame.mixer.music.play()
                        self.detected_label = {
                            "depth_diff": depth_difference,
...
class ZoomWindow(QDialog):
    ...
    def init_ui(self):
        ...
        button_layout.addWidget(self.people)

        button_layout.addWidget(self.location_lxt)
        button_layout.addWidget(self.description_des)
        button_layout.addWidget(self.disposal_dis)
        button_layout.addWidget(self.disposal_1)
...
class MyWindow(QMainWindow):
    ...
    def setupTab2(self):
        ...
        self.line_label.setFixedSize(100, 30)
        ...
        self.station_label.setFixedSize(100, 30)
        ...
        self.area_label.setFixedSize(100, 30)
        ...
        self.centre_label.setFixedSize(100, 30)
        ...
        self.centre_label1.setFixedSize(100, 30)
        ...
        self.centre_label2.setFixedSize(100, 30)
        ...
        self.model_label.setFixedSize(100, 30)

        self.model_combo = QComboBox()
        # 向下拉框添加模型选项
        model_options = ["铝板", "栅格", "冲孔板", "勾搭铝板", "拉网","铝条板","栅格加网","铝方通"]
        ...
        self.capture_interval_label.setFixedSize(100, 30)
        ...
        self.apply_exposure_button = QPushButton("曝光度")
        self.apply_exposure_button.clicked.connect(self.capture_images)
        work_layout.addWidget(self.apply_exposure_button)
        self.exposure_input = QLineEdit()
        work_layout.addWidget(self.exposure_input)

        self.alarm_label = QLabel("报警信息")
        self.alarm_label.setAlignment(Qt.AlignCenter)
        self.alarm_label.setFrameStyle(QLabel.Panel | QLabel.Sunken)
        self.alarm_label.setFixedSize(100, 30)
        alarm_layout.addWidget(self.alarm_label)


        self.alarm_label1 = QLabel("报警类型")
    ...
    def update_selected_model(self, model_index):
        ...
        self.image_label5.setPixmap(pixmap.scaled(50, 50))  # 设置图片大小
    ...
    # def setupTab5(self):
    #     filepath = 'D:/PycharmProjects/pythonProject/jieguo/result_data.txt'
