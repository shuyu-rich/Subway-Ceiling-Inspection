...
class ImageCaptureThread(QThread):
    ...
    # detectedAlarmConditions = Signal(list)  # 新增信号，用于传递符合报警条件的LC, LF, CF信息
    ...
    def load_model(self, model_name):
        model_path = 'C:/Users/20115/Desktop/yolov5'
        weight_paths = {
            '铝板': 'C:/Users/20115/Desktop/yolov5/runs/train/all/flvban.pt',
            '冲孔板': 'C:/Users/20115/Desktop/yolov5/runs/train/chongkong/weights/best.pt',
            '拉网': 'C:/Users/20115/Desktop/yolov5/runs/train/all/best.pt',
            '铝条板': 'C:/Users/20115/Desktop/yolov5/runs/train/all/ltb.pt',
            '栅格加网': 'C:/Users/20115/Desktop/yolov5/runs/train/all/shangejiawang.pt',
            '矿棉板': 'C:/Users/20115/Desktop/yolov5/runs/train/all/kuangmian.pt',
            '栅格': 'C:/Users/20115/Desktop/yolov5/runs/train/all/shange.pt',
            '铝方通': 'C:/Users/20115/Desktop/yolov5/runs/train/all/shange.pt',
            '勾搭铝板': 'C:/Users/20115/Desktop/yolov5/runs/train/all/cuowei.pt'
        }
        weight_path = weight_paths.get(model_name, 'C:/Users/20115/Desktop/yolov5/runs/train/all')
    ...
    def process_common_model(self, color_frame):
        ...
        detection_img = self.process_image(color_frame)
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            ...
            if vertical_distance3 >= 100 or vertical_distance >= 3:
            ...
            #             (0, 0, 255), 2)
        # self.detectedAlarmConditions.emit(alarm_conditions)
        # 判断是否满足报警条件
    ...
    def process_detections(self, results, color_frame):
        ...
        :return:
        '''
        global vertical_distance, vertical_list1
        image_width_px = 2000  # 图像宽度，单位：像素
        image_height_px = 1500  # 图像高度，单位：像素

        detections = results.xyxy[0].cpu().numpy()  # 转换结果为numpy数组
            ...
            for i in range(len(inside_cuofeng_boxes) - 1):
                ...
                if is_horizontal:
                    ...
                    try:
                        ...
                        #               (0, 0, 255), 2)
                        # cv2.rectangle(depth_frame, (int(c_depth_x2 - 3), int(depth_y - 3)),
                        #               (int(c_depth_x2 + 3), int(depth_y + 3)),
                        #               (0, 0, 255), 2)
                    except Exception as e:
                        print(f"超出边界，错误信息{e}")
                        vertical_distance = 0
                    ...
                    try:
                        ...
                        # vertical_distance = abs(depth_value_right_top - depth_value_left_top)
                        vertical_distance = self.calculate_vertical_distance(depth_value_right_top,
                                                                            depth_value_left_top)
                    except Exception as e:
                        print(f"超出边界，错误信息{e}")
                    ...
                    print(
                        f"纵向 - 框 {i} 和框 {i + 1} 之间的像素差值: x差 = {x_diff_mm}mm, y差 = {y_diff_mm}mm,, 裂缝宽度={vertical_list1}")

                print("*" * 80, vertical_list1, vertical_distance)
                if vertical_list1 >= 10 or vertical_distance >= 3.0:
                    print("*"*80,vertical_list1, vertical_distance)
                    self.alarmStatusChanged.emit(True)
                    self.tankuang = True
                    ...
                    pygame.mixer.music.load("./mp3/cz.mp3")  # 替换为你的音频文件路径
                    pygame.mixer.music.play()
                    self.detected_label = {
                        "LC": vertical_distance,
                ...
                # distance_text = f"lc:{vertical_distance},lf:{vertical_list2},cf:{x_diff_mm}"
    ...
    def trigger_alarm1(self):
        # 触发报警的操作，可以是发出警告声音、显示警告信息等
        print("触发报警！垂直距离大于等于三毫米。")
        pygame.mixer.music.load("D:/PycharmProjects/pythonProject/mp3/cz.mp3")  # 替换为你的音频文件路径
        pygame.mixer.music.play()
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.db_connection = sqlite3.connect("D:/PycharmProjects/pythonProject/sql/example.db")
