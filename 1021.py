...
class YourClass(QObject):
    ...
    def __init__(self):
        ...
        self.ser = None
        self.connect_to_serial("COM7")  # 自动连接到指定的串口
...
class ImageCaptureThread(QThread):
    ...
    def load_model(self, model_name):
        ...
        model_path = './yolov5'
        weight_paths = {
            '补丁铝板': './yolov5\\runs\\train\\all\\716fanglvban.pt',
            '铝板': './yolov5\\runs\\train\\all\\902fanglvban.pt',
            '反光冲孔板': './yolov5/runs/train/all/523fanguang.pt',
            '冲孔板': './yolov5/runs/train/all/620ck.pt',
            '拉网+板': './yolov5/runs/train/all/1015heibai.pt',
            '铝条板': './yolov5/runs/train/all/902ltb.pt',
            '栅格加网': './yolov5/runs/train/all/shangejiawang.pt',
            '矿棉板': './yolov5/runs/train/all/kuangmian.pt',
            '栅格': './yolov5/runs/train/all/717lvfangtong.pt',
            '铝方通': './yolov5/runs/train/all/1011lvfangtong.pt',
            '勾搭铝板': './yolov5/runs/train/all/830gouda.pt',
            '垂片': './yolov5/runs/train/all/511lvfangtong.pt',
            '高铝板': './yolov5/runs/train/all/1015gaolvban.pt',
            '宽铝方通': './yolov5/runs/train/all/1011kuanlvfangtong.pt',
            '突出铝板': './yolov5/runs/train/all/1018tuchulvban.pt',
        }
        weight_path = weight_paths.get(model_name, '.yolov5/runs/train/all')
    ...
    def process_common_model(self, color_frame):
        self.tankuang = False
        # 引用全局变量
        global global_lc_lf_conditions , depth_value_at_x2_y2, depth_value_at_x1_y1
        global_lc_lf_conditions = []
        ...
        if self.selected_model in ["铝板", "补丁铝板"]:
            ...
            if len(res) < 10:
                # 调用深色方吕版的模型
                b_path = "./yolov5\\runs\\train\\all\\shenselvban.pt"
                models1 = torch.hub.load('./yolov5', 'custom', path=b_path, source='local')
        ...
        for *xyxy, conf, cls in res:
            ...
            if float(vertical_distance3) < 0.1:
                vertical_distance3 = random.uniform(0.5, 0)
            detected_label = {
            ...
            if vertical_distance3 >= 3 or vertical_distance >= 3.0:
                ...
                if vertical_distance3 >= 10:
                    ...
                    pygame.mixer.music.load("./sql/sp.mp3")  # 替换为你的音频文件路径
            ...
            for i in range(len(inside_cuofeng_boxes) - 1):
                ...
                if is_horizontal:
                    ...
                    if float(vertical_list) < 0.1:
                        vertical_list = random.uniform(0.5, 0)
                    print(
                ...
                if vertical_list >= 10 or vertical_distance >= 3.0:
                    ...
                    pygame.mixer.music.load("./sql/cz.mp3")  # 替换为你的音频文件路径
                    pygame.mixer.music.play()
                    detected_label = {
                        "column_index": self.column_index,
                        "LC": vertical_distance,
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.db_connection = sqlite3.connect("./sql/example.db")
