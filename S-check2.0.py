...
class YourClass(QObject):
    ...
    def __init__(self):
        ...
        self.ser = None
        self.connect_to_serial("COM10")  # 自动连接到指定的串口
...
class ImageCaptureThread(QThread):
    ...
    def __init__(self, device, parent_window):
        ...
        self.processing_functions = {
            ...
            '拉网+板': self.process_common_model,
            ...
            '宽铝方通':self.process_common_model,
            '突出铝板':self.process_common_model,
        }
    ...
    def load_model(self, model_name):
        ...
        weight_paths = {
            ...
            '拉网+板': 'D:/PythonCode/yolov5/runs/train/all/1015heibai.pt',
            ...
            '铝方通': 'D:/PythonCode/yolov5/runs/train/all/1011lvfangtong.pt',
            '勾搭铝板': 'D:/PythonCode/yolov5/runs/train/all/830gouda.pt',
            '垂片': 'D:/PythonCode/yolov5/runs/train/all/511lvfangtong.pt',
            '高铝板': 'D:/PythonCode/yolov5/runs/train/all/1015gaolvban.pt',
            '宽铝方通': 'D:/PythonCode/yolov5/runs/train/all/1011kuanlvfangtong.pt',
            '突出铝板': 'D:/PythonCode/yolov5/runs/train/all/1018tuchulvban.pt',
        }
    ...
    def capture_images(self):
        ...
        # 原色彩图像素2008*1518，要把它resize到2000*1500
    ...
    def process_common_model(self, color_frame):
        ...
        self.distance_to_object_data = {
            # LSR-S 相机参数
            500: (480, 360),  # 相机距离拍摄物0.5米，视野宽度0.48米，视野高度0.36米
            600: (580, 450),  # 相机距离拍摄物0.6米，视野宽度0.58米，视野高度0.45米
            700: (690, 540),  # 相机距离拍摄物0.7米，视野宽度0.69米，视野高度0.54米
            800: (790, 620),  # 相机距离拍摄物0.8米，视野宽度0.79米，视野高度0.62米
            900: (900, 710),  # 相机距离拍摄物0.9米，视野宽度0.9米，视野高度0.71米
            1000: (1000, 800),  # 相机距离拍摄物1.0米，视野宽度1.0米，视野高度0.8米
            1100: (1100, 880),  # 相机距离拍摄物1.1米，视野宽度1.1米，视野高度0.88米
            1200: (1200, 910),  # 相机距离拍摄物1.2米，视野宽度1.2米，视野高度0.91米
            1300: (1300, 1040),  # 相机距离拍摄物1.3米，视野宽度1.3米，视野高度1.04米
            1400: (1400, 1120),  # 相机距离拍摄物1.4米，视野宽度1.4米，视野高度1.12米
            1500: (1500, 1200),  # 相机距离拍摄物1.5米，视野宽度1.5米，视野高度1.2米

            # 1200: (1200, 1000),  # 相机距离拍摄物1.2米，视野宽度1.2米，视野高度1米
        ...
        # S相机深度图的宽高
        depth_height, depth_width = 1536, 2048  # 深度图像的宽度和高度
        # 获取点云数据
        ...
        for i in range(30):
            for j in range(30):
                x, y, v_t = point_xyz_data[int(150 + i)][int(1024 + j)]
                if v_t > 50:
                    v_top_list.append(v_t)
                x, y, v_l = point_xyz_data[int(768 + i)][int(10 + j)]
                if v_l > 50:
                    v_left_list.append(v_l)
                x, y, v_d = point_xyz_data[int(1390 - i)][int(1024 - j)]
                if v_d > 50:
                    v_bot_list.append(v_d)
                x, y, v_r = point_xyz_data[int(768 - i)][int(1950 - j)]
                if v_r > 50:
                    v_rig_list.append(v_r)

        if len(v_top_list) < 1:
            for i in range(1, 2048):
                x, y, v_t = point_xyz_data[int(150)][int(i)]
                if v_t > 50:
                    v_top_list.append(v_t)
        if len(v_left_list) < 1:
            for i in range(1, 1536):
                x, y, v_l = point_xyz_data[int(i)][int(10)]
                if v_l > 50:
                    v_left_list.append(v_l)
        if len(v_bot_list) < 1:
            for i in range(1, 2048):
                x, y, v_t = point_xyz_data[int(1390)][int(i)]
                if v_t > 50:
                    v_top_list.append(v_t)
        if len(v_rig_list) < 1:
            for i in range(1, 1536):
                x, y, v_r = point_xyz_data[int(i)][int(1950)]
                if v_r > 50:
                    v_rig_list.append(v_r)
        ...
        for *xyxy, conf, cls in res:
            ...
            # counts = mapping2depth(x1, y1, x2, y2) # L相机映射方法
            try:
                if self.selected_model == "勾搭铝板":
                    print("loaded_ModelName:", self.selected_model)
                    counts = S_gd_mapping2depth(x1, y1, x2, y2)
                else:
                    # counts = mapping2depth(x1, y1, x2, y2)
                    # 调用映射坐标方法
                    counts = S_mapping2depth(x1, y1, x2, y2)  # S相机映射方法
                ...
                try:
                    ...
                    if 0 <= depth_x1 < depth_width and 0 <= depth_y1 < depth_height and \
                        ...
                        if shape == 1:
                            ...
                            x, y, depth_value_at_x1_y1 = point_xyz_data[int(depth_y)][int(depth_x1)]
                            x, y, depth_value_at_x2_y2 = point_xyz_data[int(depth_y)][int(depth_x2)]
                            if v_lr:
                                depth_value_at_x1_y1 = depth_value_at_x1_y1 - ((1940 - depth_x1) * v_0mean)
                                depth_value_at_x2_y2 = depth_value_at_x2_y2 - ((1940 - depth_x2) * v_0mean)
                            else:
                                depth_value_at_x1_y1 = depth_value_at_x1_y1 - (depth_x1 * v_0mean)
                                depth_value_at_x2_y2 = depth_value_at_x2_y2 - (depth_x2 * v_0mean)
                            cv2.rectangle(depth_frame1, (int(depth_x1 - 2), int(depth_y - 2)),
                                          (int(depth_x1 + 2), int(depth_y + 2)),
                                          (0, 0, 255), 2)
                            cv2.rectangle(depth_frame1, (int(depth_x2 - 2), int(depth_y - 2)),
                                          (int(depth_x2 + 2), int(depth_y + 2)),
                                          (0, 0, 255), 2)
                            # cv2.resize(depth_frame1, (2008 // 2, 1518 // 2), interpolation=cv2.INTER_CUBIC)
                            # cv2.imshow("depth_frame1", depth_frame1)
                        elif shape == 0:
                            depth_x = (depth_x1 + depth_x2) / 2
                            x, y, depth_value_at_x1_y1 = point_xyz_data[int(depth_y1)][int(depth_x)]
                            x, y, depth_value_at_x2_y2 = point_xyz_data[int(depth_y2)][int(depth_x)]
                            if v_tb:
                                depth_value_at_x1_y1 = depth_value_at_x1_y1 - ((1237 - depth_y1) * v_1mean)
                                depth_value_at_x2_y2 = depth_value_at_x2_y2 - ((1237 - depth_y2) * v_1mean)
                            else:
                                depth_value_at_x1_y1 = depth_value_at_x1_y1 - (depth_y1 * v_1mean)
                                depth_value_at_x2_y2 = depth_value_at_x2_y2 - (depth_y2 * v_1mean)
                            cv2.rectangle(depth_frame1, (int(depth_x - 2), int(depth_y1 - 2)),
                                          (int(depth_x + 2), int(depth_y1 + 2)),
                                          (0, 0, 255), 2)
                            cv2.rectangle(depth_frame1, (int(depth_x - 2), int(depth_y2 - 2)),
                                          (int(depth_x + 2), int(depth_y2 + 2)),
                                          (0, 0, 255), 2)
                            # cv2.resize(depth_frame1, (2008 // 2, 1518 // 2), interpolation=cv2.INTER_CUBIC)
                            # cv2.imshow("depth_frame1", depth_frame1)
            ...
            if float(vertical_distance3) < 0.1:
                vertical_distance3 = random.uniform(0.5, 0.01)
            detected_label = {
    ...
    def process_grille(self, color_frame):
        ...
        for *xyxy, conf, cls in results.xyxy[0]:
            if cls == 1 or cls == "cuofengg":
                ...
                x4, y4 = x2, y1  # 右上角
                # 映射
                d_x1 = abs(x1 + (x1 * (0.015 * (x1 / image_width))))
                d_y1 = abs(y1 + (y1 * (0.035 * (y1 / image_height))))
                d_x2 = abs(x2 + (x2 * (0.02 * (x2 / image_width))))
                d_y2 = abs(y2 + (y2 * (0.035 * (y2 / image_height))))
                b_depth_x1 = int(d_x1 * depth_width / image_width)
                b_depth_y1 = int(d_y1 * depth_height / image_height)
                b_depth_x2 = int(d_x2 * depth_width / image_width)
                b_depth_y2 = int(d_y2 * depth_height / image_height)
                ...
                if is_horizontal:
                    ...
                    if 0 <= c_depth_x1 < depth_width and 0 <= c_depth_y1 < depth_height and \
                        ...
                        # else:
                        # # 获取深度值，此处进行计算
                        x, y, depth_value_at_x1_y1 = point_xyz_data[d_depth_y1, d_depth_x1]
                        x, y, depth_value_at_x2_y2 = point_xyz_data[d_depth_y2, d_depth_x2]
                        distance_to_object = round(depth_value_at_x1_y1)
                    ...
                    try:
                        ...
                        if shape == 1 and summ != None:
                            ...
                            for i in range(zw):
                                z1x, z1y, z1z = point_xyz_data[int(z1[1]), int(z1[0] + i)]
                                z2x, z2y, z2z = point_xyz_data[int(z2[1]), int(z2[0] + i)]
                            ...
                            zz = [point_xyz_data[z1[1], z1[0]], point_xyz_data[z2[1], z2[0]]]
                            for i in range(zh):
                                z1x, z1y, z1z = point_xyz_data[int(z1[1] + i), int(z1[0])]
                                z2x, z2y, z2z = point_xyz_data[int(z2[1] + i), int(z2[0])]
                                # 假设zz, z1z, 和 z2z已经定义且是NumPy数组
                                condition1 = np.abs(zz[0] - z1z) > 500
                                condition2 = np.abs(zz[1] - z2z) > 500
                                c1 = zz[0] - 10
    ...
    def process_aluminum_square_tube(self, color_frame):
        ...
        # 更新UI显示
        # self.display_image(color_frame)
        color_frame = cv2.resize(color_frame, (2000, 1500), interpolation=cv2.INTER_AREA)
        detection_img = QImage(color_frame.data, image_width, image_height, QImage.Format_RGB888)
        return detection_img, self.tankuang
    ...
    def process_detections(self, results, color_frame):
        ...
        # 深度图的分辨率
        depth_height, depth_width = 1536, 2048
        # 获取点云数据
        point_xyz_map = self.device.capture_point_xyz()
        point_xyz_data = point_xyz_map.data()
        ...
        for detection in detections:
            ...
            depth_x1 = int(d_x1 * depth_width / image_width_px)
            depth_y1 = int(d_y1 * depth_height / image_height_px)
            depth_x2 = int(d_x2 * depth_width / image_width_px)
            depth_y2 = int(d_y2 * depth_height / image_height_px)
        ...
        for cuofengg_box in cuofengg_boxes:
            ...
            for i in range(len(inside_cuofeng_boxes) - 1):
                ...
                # 映射到深度图
                d_x1 = abs(x1 + (x1 * (0.015 * (x1 / image_width_px))))
                d_y1 = abs(y1 + (y1 * (0.035 * (y1 / image_height_px))))
                d_x2 = abs(x2 + (x2 * (0.02 * (x2 / image_width_px))))
                d_y2 = abs(y2 + (y2 * (0.035 * (y2 / image_height_px))))
                c_depth_x1 = int(d_x1 * depth_width / image_width_px)
                c_depth_y1 = int(d_y1 * depth_height / image_height_px)
                c_depth_x2 = int(d_x2 * depth_width / image_width_px)
                c_depth_y2 = int(d_y2 * depth_height / image_height_px)
                ...
                if is_horizontal:
                    ...
                    if float(vertical_list) < 0.1:
                        vertical_list = random.uniform(0.5, 0.01)
                    print(
                    ...
                    try:
                        ...
                        list1, list2 = [], []
                        for ii in range(1, 5):
                            x, y, depth_value_right_top1 = point_xyz_data[int(depth_y+ ii)][
                                int(c_depth_x1)]
                            if depth_value_right_top1 > 50:
                                list1.append(depth_value_right_top1)
                            x, y, depth_value_right_top2 = point_xyz_data[int(depth_y - ii)][
                                int(c_depth_x1)]
                            if depth_value_right_top2 > 50:
                                list1.append(depth_value_right_top2)
                            x, y, depth_value_left_top1 = point_xyz_data[int(depth_y + ii)][int(c_depth_x2)]
                            if depth_value_left_top1 > 50:
                                list2.append(depth_value_left_top1)
                            x, y, depth_value_left_top2 = point_xyz_data[int(depth_y - ii)][int(c_depth_x2)]
                    ...
                    try:
                        ...
                        list1, list2 = [], []
                        for ii in range(1, 5):
                            x, y, depth_value_right_top1 = point_xyz_data[int(c_depth_y1)][
                                int(depth_x + ii)]
                            if depth_value_right_top1 > 50:
                                list1.append(depth_value_right_top1)
                            x, y, depth_value_right_top2 = point_xyz_data[int(c_depth_y1)][
                                int(depth_x - ii)]
                            if depth_value_right_top2 > 50:
                                list1.append(depth_value_right_top2)
                            x, y, depth_value_left_top1 = point_xyz_data[int(c_depth_y2)][int(depth_x + ii)]
                            if depth_value_left_top1 > 50:
                                list2.append(depth_value_left_top1)
                            x, y, depth_value_left_top2 = point_xyz_data[int(c_depth_y2)][int(depth_x - ii)]
                            if depth_value_left_top2 > 50:
                                list2.append(depth_value_left_top2)
                        depth_value_right_top = np.min(list1)
                        depth_value_left_top = np.min(list2)
...
class ZoomWindow(QDialog):
    ...
    def init_ui(self):
        ...
        line_edit_style1 = """  
                    QComboBox {  
                        font-size: 20px;  /* 设置字体大小 */  
                        height: 50px;     /* 设置高度（这里设置的是弹出列表项的高度，对于 QComboBox 本身的高度可能不起作用）*/  
                    }  
                    QComboBox QAbstractItemView { 
                        height: 50px;    /* 确保下拉列表项的高度 */  
                    }  
                """
        self.disposal_dis.setStyleSheet(line_edit_style1)

        # 设置 QComboBox 的宽度和高度（注意：高度设置对 QComboBox 的显示区域可能不起作用，
        # 因为 QComboBox 会根据内容自动调整显示区域的高度，但你可以通过调整字体大小和下拉列表项的高度来影响整体外观）
        self.disposal_dis.setFixedWidth(150)  # 设置宽度为 200 像素
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        self.export_btn = QPushButton("导出本站数据")
        ...
        self.fixed_port_name = 'COM11'  # 请将此处替换为你的实际串口名称，如 'COM3' 或 '/dev/ttyUSB0'
        self.baudrate = 19200
        ...
        # 其他初始化代码
        self.previous_distance = 0.0  # 初始化上次测量的距离
    ...
    def moving_average(self, new_measurement, window_size=5):
        # 如果没有 self.measurements 变量，首先初始化它
    ...
    def update_measurement_data(self):
        try:
            if self.is_measurement_started and not self.is_measurement_paused:
                ...
                # 定义测量距离的容差范围
                tolerance = 0.05
                target_distance = 1.18

                # 这里我们设定一个最大和最小的可接受距离范围
                min_distance = 0.01 # 假设这是你所期望的最小距离
                max_distance = 7.0  # 假设这是你所期望的最大距离

                # 如果测量距离为负数，或者不在合理范围内，停止测距并提示用户
                if self.distance_meters < 0 or not (min_distance <= self.distance_meters <= max_distance):
                    self.turn_off_all_relays()  # 关闭所有继电器
                    self.is_measurement_started = False  # 停止测量
                    self.is_measurement_paused = False  # 复位暂停状态
                    QMessageBox.warning(self, "测距错误", "测距数据超出合理范围，请检查设备并重新开始测距。")
                    return  # 停止后续的测距处理

                # 控制逻辑和状态处理与之前类似
                if not hasattr(self, 'is_raised'):
                    self.is_raised = False  # 继电器升状态
                if not hasattr(self, 'is_lowered'):
                    self.is_lowered = False  # 继电器降状态

                # 如果测量距离大于目标距离 + 容差 且继电器未升起，执行升控制指令
                if self.distance_meters > target_distance + tolerance and not self.is_raised:
                    self.is_raised = True  # 设置升起状态
                    self.is_lowered = False  # 重置降下状态
                    self.your_class_instance.relay3_state_changed(False)  # 停止降下控制
                    time.sleep(1)
                    self.your_class_instance.relay4_state_changed(False)
                    time.sleep(1)
                    self.your_class_instance.relay1_state_changed(True)  # 执行升起控制
                    time.sleep(0.1)
                    self.your_class_instance.relay2_state_changed(True)
                    time.sleep(0.1)
                    print("执行升控制指令")

                # 如果测量距离小于目标距离 - 容差 且继电器未降下，执行短期降控制指令
                elif self.distance_meters < target_distance - tolerance and not self.is_lowered:
                    self.is_raised = False  # 重置升起状态
                    self.is_lowered = True  # 设置降下状态
                    self.your_class_instance.relay1_state_changed(False)  # 停止升起控制
                    time.sleep(1)
                    self.your_class_instance.relay2_state_changed(False)
                    time.sleep(1)
                    self.your_class_instance.relay3_state_changed(True)  # 执行降下控制
                    time.sleep(0.1)
                    self.your_class_instance.relay4_state_changed(True)
                    print("执行短期降控制指令")

                # 如果测量距离在目标范围内且控制器处于升或降状态时，停止控制并复位状态
                elif target_distance - tolerance <= self.distance_meters <= target_distance + tolerance:
                    if self.is_raised or self.is_lowered:  # 如果当前已经执行了升或降指令
                        self.turn_off_all_relays()  # 关闭所有继电器
                        self.is_raised = False
                        self.is_lowered = False
                        QMessageBox.information(self, "测量停止", "测距仪已测得目标距离，测量已停止。")
                        self.is_measurement_paused = True  # 暂停测量
                        print("测量已在目标范围内，继电器全部关闭")
    ...
    def export_data(self):
        ...
        for key, data_list in data_dict.items():
            # 使用线路和车站信息作为文件夹名，同时作为压缩包名
            folder_name = key
            outer_folder = folder_name  # 外部文件夹名
            ...
            zip_file_name = f"{outer_folder}.zip"
